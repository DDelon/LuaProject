...
ResultAnimate.RESOURCE_FILENAME = SmallGamesGF.getCurAppResUIPath("main.uiResult2.lua")
ResultAnimate.RESOURCE_BINDING  = {    
    ...
    btn_fx            = { parent = "panel", varname = "btn_fx", events={event=SmallGamesGI.bindingEvents.ClickScale, method="onClickShare"} },
...
function ResultAnimate:onClickClose( ... )
    self:play("end_", false)
    local delayCall  = {}
    delayCall[#delayCall + 1]  = cc.DelayTime:create(0.25)
    delayCall[#delayCall + 1]  = cc.CallFunc:create(function ( ... )
...
function ResultAnimate:onTouchBegan( ... )
    if self.exitable then
        self.exitable = false
        ...
        delayCall[#delayCall + 1]  = cc.DelayTime:create(0.5)
        delayCall[#delayCall + 1]  = cc.CallFunc:create(function ( ... )
            self:getParent():removeFromParent()
            self.funcEnd()
        end)

        self:runAction(transition.sequence(delayCall))
    end
...
function ResultAnimate:onClickShare()
    SmallGamesGF.onShareWechatImage()
end
